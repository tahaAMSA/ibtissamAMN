  ...
  return (
    <div className='relative'>
        ...
        <span className='hidden text-right lg:block'>
          <span className='block text-sm font-medium dark:text-white'>
            {user.firstName && user.lastName 
              ? `${user.firstName} ${user.lastName}${user.firstNameAr && user.lastNameAr ? ` (${user.firstNameAr} ${user.lastNameAr})` : ''}`
              : user.firstName || user.lastName || user.username || 'Utilisateur'
            }
          </span>
      ...
      <div
        ref={dropdown}
        className={cn(
          'absolute right-0 mt-4 flex w-80 flex-col rounded-lg border border-gray-200 bg-white shadow-lg dark:border-strokedark dark:bg-boxdark dark:text-white',
          {
            hidden: !dropdownOpen,
          }
        )}
